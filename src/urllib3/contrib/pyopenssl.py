...
# ********************************************************************************************************************
        ...
        if self.check_dane:
            try:
                ...
                from .dnssec.secdns import get_validated_record
                from dns import rdatatype, resolver

                try:
                    proto = 'tcp' if sock.type == SOCK_STREAM else 'udp'
                    secdns = get_validated_record('_' + str(sock.getpeername()[1]) + '._' + proto + '.' + server_hostname.decode('utf-8'), rdatatype.TLSA)
                    if secdns != False:
                        # success
                        for record in secdns:
                            tpl = record.to_text().split(' ')[-5:]

                            try:
                                self.dane_tlsa_records.append((int(tpl[0]),
                                                               int(tpl[1]),
                                                               int(tpl[2]),
                                                               bytes.fromhex(tpl[3])))
                            except ValueError:
                                pass # Ignore if call was invalid
                    else:
                        raise DANEError("DNSSEC validation failed.")
                    ...
                    if ret != 1:
                        raise DANEError("Could not enable DANE CTX.")

                    ret = cnx.set_dane_enable()  # enable dane
                    if ret != 1:
                        raise DANEError("Could not enable DANE.")

                    # add all the tlsa records to be used
                    for record in self.dane_tlsa_records:
                        ret = cnx.dane_tlsa_add(*record)
                        if ret != 1:
                            raise DANEError("Could not add TLSA record.")

                except resolver.NXDOMAIN:
                    raise DANEError("The requested domain has no TLSA records.")
